...
print("Sending waveform on pin "+sys.argv[1])
...
pi.wave_add_generic(frameToWave(frame_on)) # wave_on
wave_on_id = pi.wave_create() # create and save id

pi.wave_add_generic(frameToWave(frame_off)) # wave_on
wave_off_id = pi.wave_create() # create and save id

while True:
    print("Sending ON")
    pi.wave_send_repeat(wave_on_id)
    time.sleep(1)
    pi.wave_tx_stop() # stop waveform
    print("Sending OFF")
    pi.wave_send_repeat(wave_off_id)
    time.sleep(1)
    pi.wave_tx_stop() # stop waveform
