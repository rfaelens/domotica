...
#        pkt = self.socket.recv(
    ...
    def process_packet(self, pkt):
        ...
        if not ( \
            ((self.mode & ScannerMode.MODE_IBEACON) and (pkt[20:24] == b"\x4c\x00\x02\x15")) or \
            ((self.mode & ScannerMode.MODE_EDDYSTONE) and (pkt[20:22] == b"\xaa\xfe")) or \
            ((self.mode & ScannerMode.MODE_ESTIMOTE) and (pkt[20:22] == b"\x9a\xfe"))):
            return

        bt_addr = bt_addr_to_string(pkt[7:13])
        rssi = bin_to_int(pkt[-1])
        # strip bluetooth address and parse packet
        packet = parse_packet(pkt[18:-1])
